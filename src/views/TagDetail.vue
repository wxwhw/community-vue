  ...
  <div class="main">
    <el-container>
      ...
      <el-main>
        <div class="main-box">
          <div class="top-info">
            <h2 style="text-align: center; margin: 20px; ">{{ tag.name }}</h2>
            <p style="text-align: center;">{{ tag.description }}</p>
          </div>
...
export default {
  ...
  // mounted: function () {
  //   this.$watch('$route.params.id', function (newId, oldId) {
  //     if (newId !== oldId) {
  //       this.tagId = this.$route.params.id
  //       this.loadArticleList()
  //       this.loadTag()
  ...
  methods: {
    ...
    loadArticleList() {
      return this.request.get('http://localhost:9505/article/getArticleByTagId', {
        params: {
    ...
    loadTag() {
      this.request.get(`http://localhost:9505/tag/${this.tagId}`).then(res => {
        if (res.code === 200) {
          this.tag = res.result
        } else {
